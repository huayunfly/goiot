 ...
 * load drivers, read/write/refresh data on Linux.
 *
 * @author Yun Hua
 * @version 0.1 2025.07.07
 */

#include "driver_service.h"

#include <dirent.h>  // Linux file manipulation.
...
namespace goiot {
	...
	const std::string DriverMgrService::CONFIG_FILE = "drivers.json";
	const std::string DriverMgrService::DRIVER_DIR = "drivers";
	...
	const Json::StaticString DriverMgrService::J_KEY_NAME{ "name" };
	const Json::StaticString DriverMgrService::J_KEY_REDIS{ "redis" };
	const Json::StaticString DriverMgrService::J_KEY_DRIVERS{ "drivers" };
	const Json::StaticString DriverMgrService::J_KEY_TYPE{ "type" };
	const Json::StaticString DriverMgrService::J_KEY_PORT{ "port" };
	...
	int DriverMgrService::LoadJsonConfig()
	{
		...
		const std::string name = root[J_KEY_NAME].asString();
		// parse redis connection string
		if (root[J_KEY_REDIS])
		{
			std::string redis_string = root[J_KEY_REDIS].asString();
		...
		// driver descriptions
		assert(root[J_KEY_DRIVERS].isArray());
		driver_descriptions_.clear();
		for (auto driver : root[J_KEY_DRIVERS])
		{
			auto description = std::make_tuple(driver[J_KEY_TYPE].asString(), driver[J_KEY_PORT].asString(), driver.toStyledString());
			if (std::find_if(driver_descriptions_.begin(), driver_descriptions_.end(),
				[&driver](std::shared_ptr<std::tuple<std::string, std::string, std::string>> pos) {
					return std::get<0>(*pos) == driver[J_KEY_TYPE].asString() && std::get<1>(*pos) == driver[J_KEY_PORT].asString(); }) == driver_descriptions_.end())
			{
		...
		const std::vector<std::shared_ptr<std::tuple<std::string/*type*/, std::string/*port*/, std::string/*content*/>>>& driver_descriptions) 
	{
		...
		modules.clear();
		// Get file names
		std::vector<std::string> filenames = GetFilePathNames(module_path + DRIVER_DIR);
		if (filenames.empty())
		{
		...
		{
			if (item.rfind(".so") == std::string::npos)
			{
				continue;
			}
			// Load the library
			HINSTANCE module = dlopen(item.c_str(), RTLD_LAZY);
			if (!module)
			{
			...
			dlerror();
			ObjProc obj_func = (ObjProc)dlsym(module, "_Z6GetObjv");
			NameProc name_func = (NameProc)dlsym(module, "_Z7GetNameB5cxx11v");

			if (!obj_func || !name_func)
			{
			...
			std::string driver_name = name_func();
	...
	std::vector<std::string> DriverMgrService::GetFilePathNames(const std::string& dir_path)
	{
		std::vector<std::string> files;
		DIR* s_dir = opendir(dir_path.c_str());
		struct dirent* fd;
		fd = readdir(s_dir);
		while (fd != NULL) {
			if (fd->d_type != DT_DIR) 
			{
				files.push_back(dir_path + "/" + fd->d_name);
			}
			fd = readdir(s_dir); // return the next ffd
		}
		return files;
	}
		...
		while (keep_poll_)
		{
			if (ConnectedRedis(redis_poll_))
			{
				...
				for (auto& member : member_vec)
				{
					...
					{
						uint8_t new_value = std::atoi(value.c_str());
