...
namespace goiot {
	...
	public:
		...
		DriverMgrService(const std::string& module_path) : module_path_(module_path), 
	...
	private:
		...
		// Get the file path and names under the specified directory.
		std::vector<std::string> GetFilePathNames(const std::string& dir_path);
	...
	private:
		...
		const static std::string DRIVER_DIR;
		...
		const static Json::StaticString J_KEY_REDIS;
		const static Json::StaticString J_KEY_DRIVERS;
		const static Json::StaticString J_KEY_TYPE;
