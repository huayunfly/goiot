...
namespace goiot
{
	...
	// modbus DWord decode format
	enum class DWordDecode
	{
		ABCD = 0, // without conversion
		DCBA = 1, // inversed format
		BADC = 2, // swapped bytes
	...
	struct ConnectionInfo
	{
		ConnectionInfo() : port("COM1"), baud(9600), parity('N'), 
			data_bit(8), stop_bit(1), rack(0), slot(0), response_timeout_msec(500), refresh_interval_msec(500)
		{
		}
		std::string port;
		int baud;
		...
		int rack;
		int slot;
		uint32_t response_timeout_msec; // in micro second
		uint32_t refresh_interval_msec; // in micro second
	};
	...
	struct DataInfo
	{
		...
		DWordDecode dword_decode; // DWord decode with value 0(ABCD), 1(DCBA), 2(BADC), 3(CDAB)
