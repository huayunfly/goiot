...
namespace goiot 
{
    const Json::StaticString FpDriver::J_KEY_ID{ "id" };
    const Json::StaticString FpDriver::J_KEY_NAME{ "name" };
    const Json::StaticString FpDriver::J_KEY_PORT{ "port" };

    const Json::StaticString FpDriver::J_KEY_RESPONSE_TIMEOUT{ "response_timeout_msec" };
    const Json::StaticString FpDriver::J_KEY_REFRESH_INTERVAL{ "refresh_interval_msec" };
    const Json::StaticString FpDriver::J_KEY_NODES{ "nodes" };
    const Json::StaticString FpDriver::J_KEY_DATA{ "data" };
    const Json::StaticString FpDriver::J_KEY_ADDRESS{ "address" };
    const Json::StaticString FpDriver::J_KEY_RATIO{ "ratio" };
    const Json::StaticString FpDriver::J_KEY_OFFSET{ "offset" };
    const Json::StaticString FpDriver::J_KEY_REGISTER{ "register" };
        ...
        ConnectionInfo& connection_info, std::map<std::string, DataInfo>& data_map)
    {
        ...
        // Driver ID
        _id = root[J_KEY_ID].asString();

        // Split port, like "/dev/ttyUSB0,9600,N,8,1"
        ...
        for (auto node : root[J_KEY_NODES])
        {
            ...
            for (auto data_node : node[J_KEY_DATA])
            {
                ...
                data_info.timestamp = std::chrono::duration_cast<std::chrono::milliseconds>(
                    std::chrono::system_clock::now().time_since_epoch()).count() / 1000.0;
                // If data_node[J_KEY_RATIO] is not given, it returns a Json::Value while isNull() == true, and isDouble() == false.
                if (data_node[J_KEY_RATIO].isDouble()) 
                {
                ...
                data_info.offset = data_node[J_KEY_OFFSET].asFloat(); // The offset token may missing, asFloat() returns 0.0 by default.
